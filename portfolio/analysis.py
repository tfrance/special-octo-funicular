...
def get_portfolio_value(prices, allocs, start_val=1):
    ...
    """
    # Normalize the data by dividing each element by the first row
    prices = prices / prices.iloc[0, :]

    # Update values of all of the prices by allocation weights
    prices = prices * allocs

    # Sum the asset values by row for daily portfolio value:
    prices = prices.sum(axis=1)

    port_val = prices * start_val
...
def test_run():
    """Driver function."""
    # Define input paramecomp
    start_date = '2010-01-01'
    end_date = '2010-12-31'

    symbol_allocations = OrderedDict(
        [('GOOG', 0.2), ('AAPL', 0.2), ('GLD', 0.4), ('XOM', 0.2)])  # symbols and corresponding allocations
    # symbol_allocations = OrderedDict([('AXP', 0.0), ('HPQ', 0.0), ('IBM', 0.0), ('HNZ', 1.0)])  # allocations from wiki example

    symbols = list(symbol_allocations.keys())  # list of symbols, e.g.: ['GOOG', 'AAPL', 'GLD', 'XOM']
    allocs = list(symbol_allocations.values())  # list of allocations, e.g.: [0.2, 0.3, 0.4, 0.1]
